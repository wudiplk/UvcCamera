...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
    ...
    private static final void addSize(final JSONObject format, final int formatType, final List<Size> size_list) throws JSONException {
        ...
        for (int j = 0; j < size_nums; j++) {
            ...
            try {
                size_list.add(new Size(formatType, 0, j, Integer.parseInt(sz[0]), Integer.parseInt(sz[1])));
            } catch (final Exception e) {
                break;
            }
        }
